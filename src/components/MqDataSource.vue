...
const isDrawerOpen = inject('mqDrawerOpen', false) // 父页面传递的数据，抽屉是否打开
...
const drawerOpen = () =>{
	if(isCreateDataSource.value){
		...
		console.log(dataSource.ext_info)
	...
	<el-drawer ref="drawer" v-model="isDrawerOpen"  :title="drawerTitle" :destroy-on-close = "true" @open="drawerOpen" @opened="drawerOpened" size="50%">
		<el-main>
			<el-form ref='form' :model="dataSource" label-position="top" label-width="100px">
				...
				<el-form-item prop="ext_info.host" label="host(broker地址)" :rules="rules.host">
					<el-input v-model="dataSource.ext_info.host"/>
				</el-form-item>
				<el-form-item prop="ext_info.virtual_host" label="虚拟地址(virtual host)" :rules="rules.virtual_host">
					<el-input v-model="dataSource.ext_info.virtual_host"/>
				</el-form-item>
				<el-form-item prop="ext_info.exchange" label="消息交换器(exchange)" :rules="rules.exchange">
					<el-input v-model="dataSource.ext_info.exchange"/>
				</el-form-item>
				<el-form-item prop="ext_info.queue" label="队列(queue)" :rules="rules.queue">
					<el-input v-model="dataSource.ext_info.queue"/>
				</el-form-item>
